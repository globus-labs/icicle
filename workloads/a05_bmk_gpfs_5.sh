...
run_five() {
  ...
  local topic5="${TOPIC_PREFIX_HOST5}-${partitions}p"
  ...
  for i in $(seq 1 "$REPEAT"); do
    ...
    ssh -o StrictHostKeyChecking=no -i "$KEY" ubuntu@"$HOST5" "
      echo \"[HOST5] ${exp_label} topic=${topic5} run=${i}\" >&2
      cd /home/ubuntu/icicle &&
      export PYTHONPATH=. &&
      /home/ubuntu/icicle/.venv/bin/python -m monitor.main \
        --fs_type ibm \
...
for p in "${PARTITIONS_LIST[@]}"; do
  ...
  run_five "Exp1 (changelog_mode=true)" true  "$p"

  # EXP 3: changelog_mode=false
  run_five "Exp3 (changelog_mode=false)" false "$p"

  # EXP 4: changelog_mode=false + ignore open/close + reduction
  run_five "Exp4 (ignore open/close + reduction)" false "$p" \
