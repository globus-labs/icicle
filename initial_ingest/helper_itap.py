...
# Note 1: The “Multiplexer hanging up” exceptions may look concerning, but they are harmless and can be ignored.
# Note 2: use s3a:// rather than s3://
# Note 3: itap/1m/*.csv schema -- type (0), mode (1), uid (2), gid (3), size (4), atime (5), ctime (6), mtime (7), path (8)
# Note 4: search client id and secret are stored in search.py, this is not secure, in production use AWS Secrets Manager
# Note 5: Amazon Managed Flink may not call ProcessFunction.close()
# Note 6: .shuffle()/.rebalance() calls:  multiple runs of the primary pipeline will have higher chance of ingest all records
...
def itap_pipelines(conf: IngestConfig, env: StreamExecutionEnvironment) -> None:
