...
def create_spark(
    app_name: str = 'FsCsvProcess',
    mem_fraction: float = 1,
) -> SparkSession:
...
def write_sorted_values(
    ...
    metric_array = F.array(
        *[
            F.struct(F.lit(m).alias('metric'), F.col(m).alias('value'))
            for m in METRICS
        ],
    )
    ...
    long_df = df.select(
        F.col(group_col),
        F.explode(metric_array).alias('m'),
    ).select(
        F.col(group_col),
    ...
    ranked = long_df.withColumn(
        'rank_1based',
        F.row_number().over(w_group_metric),
    )
    ...
    out_dir = os.path.join(
        base_dir,
        f'{dataset_name}-{group_col}-sorted-parquet',
    )
    (
    ...
    print(
        f'[{dataset_name}] Sorted exact values written to Parquet dir: {out_dir}',
    )
...
def process_dataset(
    ...
    if dataset_name.lower() in ('itap', 'hpss'):
        ...
        raise ValueError(
            f'Unknown dataset_name for schema selection: {dataset_name}',
        )
