...
def build_progress_path(
    input_path: Path,
    rows_per_csv: int | None,
) -> Path | None:
    if rows_per_csv is None:
        return None
    progress_name = f'{input_path.stem}_rows{rows_per_csv}.progress'
    return input_path.with_name(progress_name)


def load_progress(progress_path: Path) -> tuple[int, Path | None, int]:
    try:
        payload = json.loads(progress_path.read_text())
    except (OSError, ValueError, json.JSONDecodeError):
        print(
            f'WARN: Unable to read progress file {progress_path}; ignoring it.',
        )
        return 0, None, 0

    last_line = int(payload.get('last_line', 0))
    csv_files_written = int(payload.get('csv_files_written', 0))
    db_path = payload.get('last_db_path')
    return (
        max(last_line, 0),
        Path(db_path) if db_path else None,
        max(csv_files_written, 0),
    )
...
def write_progress(
    ...
    payload = {
        'last_line': last_line,
        'last_db_path': str(last_db_path) if last_db_path else None,
        'csv_files_written': csv_files_written,
    }
    progress_path.write_text(json.dumps(payload, indent=2))


def detect_existing_csv_index(output_dir: Path) -> int:
    max_index = 0
    ...
    with path.open(encoding='utf-8', errors='surrogateescape') as handle:
        for line_num, raw in enumerate(handle, start=1):
            if _contains_surrogate_characters(raw):
                print(
                    f'WARN: Skipping line {line_num} in {path} due to undecodable bytes.',
                )
                continue
            stripped = raw.strip()
            ...
            if len(parts) != 2:
                raise ValueError(
                    f'Malformed line {line_num} in {path}: {raw.rstrip()}',
                )
...
def ensure_integer_columns(
    ...
    for column in columns:
        if column in df.columns:
            df[column] = pd.to_numeric(df[column], errors='raise').astype(
                'int64',
            )
    return df
...
def load_db_rows(
    ...
    *,
    ...
    try:
        ...
        raise sqlite3.Error(
            f'Failed to read entries table from {db_path}: {exc}',
        ) from exc

    missing_cols = [col for col in ENTRIES_COLUMNS if col not in entry_columns]
    if missing_cols:
        raise ValueError(
            f'Missing required columns {missing_cols} in entries table for {db_path}',
        )
    ...
    suffix_end = (
        len(path_str) - len(filename_suffix)
        if filename_suffix
        else len(path_str)
    )
    ...
    for row_data in replacement_rows:
        for column in ENTRIES_COLUMNS:
            assert row_data.get(column) is not None, (
                f"Null value found in column '{column}' for {db_path}"
            )
...
def build_dataframe(rows: list[dict]) -> pd.DataFrame:
    ...
    for column in df.columns:
        assert not df[column].isna().any(), (
            f"Null value(s) found in column '{column}'."
        )
...
def main() -> None:
    ...
    if filename_prefix is None or filename_suffix is None:
        raise SystemExit(
            '--filename-prefix and --filename-suffix are required.',
        )

    progress_path = build_progress_path(input_path, rows_per_csv)
    resume_from_line = 1
    last_processed_line = 0
    last_processed_db: Path | None = None
    ...
    if rows_per_csv is not None:
        ...
        csv_files_written = detect_existing_csv_index(output_dir)
        if progress_path and progress_path.exists():
            stored_line, stored_db_path, stored_csv_count = load_progress(
                progress_path,
            )
            if stored_line > 0:
                resume_from_line = stored_line + 1
                last_processed_line = stored_line
                last_processed_db = stored_db_path
                print(
                    f'INFO: Resuming from listing line {resume_from_line} using {progress_path}',
                )
            csv_files_written = max(csv_files_written, stored_csv_count)

    for index, (line_num, db_path, declared_size) in enumerate(
        read_db_listing(input_path),
    ):
        ...
        if rows_per_csv is not None and current_batch_rows >= rows_per_csv:
            ...
            print(
                f'INFO: last processed listing line {last_processed_line} -> {last_processed_db}',
            )
    ...
    if entries_frames:
        ...
        print(
            'No entry rows remain in memory (all written to CSV or no data).',
        )
