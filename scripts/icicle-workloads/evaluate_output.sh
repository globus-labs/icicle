...
usage() {
  cat <<'HELP'
Usage: evaluate_output.sh [OPTIONS]

Evaluate filesystem output: create/modify/rename/mkdir/move/delete loop.
...
Options:
  ...
  -s SLEEP     Seconds between each step (default: 0)
...
print_summary() {
  ...
  echo "  Operations:           $(( total_completed * 6 )) total (6 ops/iter)"
...
rename_file() {
  mv "hello_${1}.txt" "hi_${1}.txt"
  (( OUTPUT )) && echo "[rename]   hello_${1}.txt -> hi_${1}.txt"
}

make_dir() {
  mkdir "okdir_${1}"
  (( OUTPUT )) && echo "[mkdir]    okdir_${1}"
}

move_file() {
  mv "hi_${1}.txt" "okdir_${1}/"
  (( OUTPUT )) && echo "[move]     hi_${1}.txt -> okdir_${1}/"
}

delete_dir() {
  rm -rf "okdir_${1}"
  (( OUTPUT )) && echo "[delete]   okdir_${1}"
}
