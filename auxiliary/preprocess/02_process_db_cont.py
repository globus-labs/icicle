...
def build_progress_path(input_path: Path, rows_per_csv: Optional[int]) -> Optional[Path]:
...
def load_progress(progress_path: Path) -> tuple[int, Optional[Path], int]:
    try:
        payload = json.loads(progress_path.read_text())
    except (OSError, ValueError, json.JSONDecodeError):
        print(f"WARN: Unable to read progress file {progress_path}; ignoring it.")
        return 0, None, 0
    ...
    return max(last_line, 0), Path(db_path) if db_path else None, max(csv_files_written, 0)


def write_progress(
    progress_path: Optional[Path],
    last_line: int,
    last_db_path: Optional[Path],
...
def main() -> None:
    ...
    last_processed_db: Optional[Path] = None
    ...
    if rows_per_csv is not None:
        ...
        if progress_path and progress_path.exists():
            stored_line, stored_db_path, stored_csv_count = load_progress(progress_path)
            if stored_line > 0:
                resume_from_line = stored_line + 1
                last_processed_line = stored_line
                last_processed_db = stored_db_path
                print(f"INFO: Resuming from listing line {resume_from_line} using {progress_path}")
    ...
    for index, (line_num, db_path, declared_size) in enumerate(read_db_listing(input_path)):
        ...
        if rows_per_csv is not None and current_batch_rows >= rows_per_csv:
            ...
            csv_files_written += 1
            write_progress(progress_path, last_processed_line, last_processed_db, csv_files_written)
            print(f"INFO: last processed listing line {last_processed_line} -> {last_processed_db}")
