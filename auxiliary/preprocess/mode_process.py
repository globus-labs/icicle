...
def sym_to_mode(s: str) -> int:
    ...
    allowed = [
        set("r-"), set("w-"), set("x-") | set("sS"),
        set("r-"), set("w-"), set("x-") | set("sS"),
        set("r-"), set("w-"), set("x-") | set("tT"),
    ]
...
def explicit_tests():
    ...
    for sym, full_oct, sp_oct in cases:
        ...
        assert (m & 0o177777) == full_oct, f"{sym}: full bits mismatch {oct(m)} != {oct(full_oct)}"
        assert (m & 0o7777) == sp_oct, f"{sym}: perm+special mismatch {oct(m & 0o7777)} != {oct(sp_oct)}"
