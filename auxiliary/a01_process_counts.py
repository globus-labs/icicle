...
def write_worker_csv(
    ...
    with open(output_file, mode="w", encoding="utf-8") as file:
        ...
        for (prefix, worker), count in sorted(
            prefixes.items(), key=lambda x: (x[0][0].count("/"), x[0], x[1])
        ):
...
def write_verification_csv(
    ...
    with open(output_file, mode="w", encoding="utf-8") as file:
        ...
        for prefix, count in sorted(
            total_prefixes.items(), key=lambda x: (x[0].count("/"), x[0], x[1])
        ):
...
def expand_prefixes(prefixes, prefix_min, prefix_max):
    ...
    for i in range(len(levels) - 2, -1, -1):  # for the second to the last level
...
def shrink_workers(prefixes):
    ...
    for (name, worker), count in prefixes.items():
        ...
        new_prefixes[(name, worker)] = new_prefixes.get((name, worker), 0) + count
    return new_prefixes


def consume_to_csv_with_worker(
    request_topic: str, result_csv: str, verify_csv: str
) -> None:
    ...
    # Print statistics
    print(f"# of users-worker: {len(users)}, total files: {sum(users.values())}")
    print(f"# of groups-worker: {len(groups)}, total files: {sum(groups.values())}")
    ...
    total_users, total_groups, total_prefixes = aggregate_counts_total(
        users, groups, prefixes
    )

    # Write both CSV files
    write_worker_csv(users, groups, prefixes, result_csv)
    write_verification_csv(total_users, total_groups, total_prefixes, verify_csv)


# Example usage
request_topic = "search-alpha-counts"
# result_csv = "itap.agg2.csv"
# verify_csv = "itap.veri.csv"
result_csv = "nersc.agg2.csv"
verify_csv = "nersc.veri.csv"
# result_csv = "hpss.agg2.csv"
# verify_csv = "hpss.veri.csv"
# consume_to_csv_with_worker(request_topic, result_csv, verify_csv)
